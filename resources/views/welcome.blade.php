...
.empty-row{
  background-color: rgba(255, 229, 191, 0.829);
}
          ...
          <div id="content" class="p-4 p-md-5 pt-5" >
            ...
            @auth
              <nav class="navbar navbar-light bg-light ">
                  ...
                  <li class=" dropdown">
                      <a href="#profile"  data-toggle="collapse" aria-expanded="" class="toggle" >
             ...
             <br>
              ...
              <div class="row  float-end" >
                ...
                <div class="col-6">
                  ...
                  <select id="category" id="category" class="float-end tw-bg-gray-50 tw-border tw-border-gray-300 tw-text-gray-900 tw-text-sm tw-rounded-lg tw-focus:ring-blue-500 tw-focus:border-blue-500 tw-block tw-w-100 tw-p-2.5 tw-dark:bg-gray-700 tw-dark:border-gray-600 tw-dark:placeholder-gray-400 tw-dark:text-white tw-dark:focus:ring-blue-500 tw-dark:focus:border-blue-500">
...
<style> .toast {   position: fixed;    top: 10px; right: 10px;width: 2000px;    }</style>
          ...
          <form method="POST" action="{{ route('member.update', $post->id) }}">
              ...
              <div class="row">
                    ...
                    <div class="mb-3 col-4">
                     ...
                     <i><span class="error" style="color: red">The price field format is invalid.</span></i>
                     @endif
                 </div>
                 ...
                 <label for="exampleInputPassword1" class="lastname" id = "lastname" name = "lastname">Additionnal_ips</label>
                   <input type="text" class="form-control" id="Additionnal_ips1" name="Additionnal_ips1" value = {{$post->Additionnal_ips}}>

                 @if ($errors->has('Additionnal_ips1'))
                 <i><span class="error" style="color: red">The Additionnal ips field format is invalid.</span></i>
                 @endif
          ...
          <button data-id="{{ $post->id }}"  class="btn btn-primary btn-load"  data-loading-text="Please wait...">Test Connexion</button>
              ...
              <button class="btn btn-success" type = "submit" ><i class="fa fa-check-square-o"></i>Update</button>
                ...
                <div class="d-flex">
                  ...
                  <p>
                    Displaying {{$employees->count()}} of {{ $employees->total() }} Server(s).
                </p>
              </div>
              </div>
  ...
  <div class="vh-100 d-flex justify-content-center align-items-center" >
     <div class="modal fade" id="exampleModalCenter" tabindex="-1" aria-labelledby="exampleModalCenterTitle" role="dialog" 
         ...
         <div class="modal-dialog modal-dialog-centered">
             <div class="modal-content">
                 <div class="modal-header">
                     <h5 class="modal-title" id="exampleModalCenterTitle">Please Add a Provider</h5>
                     <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                 </div>
...
@endif
                       ...
                       <div class="mb-3">
                         <label for="exampleInputPassword1" class="form-label">Name in provider</label>
                         <input type="text" class="form-control" id="exampleInputPassword1" name="Name_Provider">
                       </div>
                       <div class="mb-3">
      ...
      <div class="col-8 mx-auto bg-white">
        ...
        <div class="modal right" id="empModal" tabindex="-1" aria-labelledby="rightModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                    <h5 class="modal-title" id="rightModalLabel">More Info on the Server</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                      <!-- Employees List -->
         ...
         <div class="vh-100 d-flex justify-content-center align-items-center">
          <div class="modal fade" id="bulk_add" tabindex="-1" aria-labelledby="exampleModalCenterTitle"
              ...
              <div class="modal-dialog modal-dialog-centered">
                  <div class="modal-content">
                      <div class="modal-header">
                          <h5 class="modal-title" id="exampleModalCenterTitle">Please Add Some Servers</h5>
                          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                      </div>
                      <form action="{{ route('Add_in_Bulk') }}" method="POST">
                        <div class="modal-body">
                          ...
                          @csrf
                            ...
                            <form>
   <div class="tw-w-full tw-mb-4 tw-border tw-border-gray-200 tw-rounded-lg tw-bg-gray-50 tw-dark:bg-gray-700 tw-dark:border-gray-600" style="border:#D3D3D3 solid 1px">
       <div id = "white"class="tw-px-4 tw-py-2 tw-bg-white tw-rounded-t-lg tw-dark:bg-gray-800">
           <label for="comment" class="sr-only">Enter Ips here one below one.</label>
...
console.log(valuesArray);
   ...
   <script>
    document.getElementById("link_ip").addEventListener("click", function() {
    const textarea = document.getElementById('message');
   ...
   var ele = document.getElementById("currency3");
    ...
    select.addEventListener("change", function() {
        ...
        if(selectedOption == "Payment_Method"){
          ...
          select3.replaceWith(select2);
          select4.replaceWith(select2);
          inputDATE.replaceWith(select2);
          inputDATE2.replaceWith(select2);
        }
        else if(selectedOption == "TEAM5"){
          input.replaceWith(select3);
          select2.replaceWith(select3);
          select4.replaceWith(select3);
          inputDATE.replaceWith(select3);
          ...
          if(share.data.length>0)
          {
            for(let i =0;i<share.data.length;i++){
              ...
              html+='<tr>\
                ...
                <td>'+share.data[i]['Password']+'</td>\
                <td>'+share.data[i]['Due_Date']+'</td>\
                <td>'+share.data[i]['Date_Creation']+'</td>\
...
' <td>' + "<a href='#edit{{$post->id}}' data-bs-toggle='modal' class='btn btn-success'><i class='fa fa-edit'></i></a> " + '</td>\n'
            ...
            html +='<tr>\
              <td align="center" colspan="10">No Servers found</td>\
              </tr>';
          }
...
$('#sel_depart88').change(function(){

   var id = $(this).val();

   document.getElementById('provider_input').value= id;

   $('#sel_emp22').find('option').not(':first').remove();

   $.ajax({
     url: '/welcome/getEmployees/'+id,
     type: 'get',
     dataType: 'json',
     success: function(response){
          var len = 0;
...
$("#attrib").change(function(){
      var valuesArray = [];
      var myDiv = document.getElementById("attrib");
      var selectElement = myDiv.getElementsByTagName("select");

      for (var i = 0; i < selectElement.length; i++) {
      var selectedOption = selectElement[i].value;

      valuesArray.push(selectedOption);
      }

      document.getElementById('atton').value= valuesArray;
      });
...
$error == "The price1 field format is invalid."||
$error == "The due  date1 field must be a date after date  creation1."||
$error == "The additionnal ips1 field format is invalid."
)
<script type="text/javascript">
  $( document ).ready(function() {
...
@endif
    ...
    (function($) {
      ...
      var firstCellContent = row.querySelector('td:first-child').textContent;

      document.getElementById('result').innerHTML= firstCellContent;
      }
      }
      ...
      divID = document.getElementById('sel2');
      allInputs = divID.getElementsByTagName('input');

      divID2 = document.getElementById('sel3');
      allsubdomains = divID.getElementsByTagName('select');
        ...
        var inputList2 = Array.prototype.slice.call(allsubdomains);
          ...
          console.log(fruits);
          console.log(fruits2);
          document.getElementById("fruits2").value = fruits2;
          document.getElementById("additional_ips").value = fruits;
      }
...
$(".btn-load").click(function (event) {
  ...
  success: function (data) {
  handleData(data);
  btn.prop('disabled', false).html('Test Connexion');
  }
  });
  function handleData(data) {
    console.log(data);
    ...
    for (var i = 0; i < contentToasts.length; i++) {
      contentToasts[i].innerHTML = data.tat;
...
<script>
  $('.TY').on('hidden.bs.modal', function() {   $('.toast').toast('hide'); });
</script>
